    ...
    <Fragment>
      ...
      >
        ...
        <p className='text-gray-400 flex'>
          <span className='text-lg font-bold'>Gourav</span> <DiTechcrunch />
        </p>
        <div className='h-full flex gap-4'>
          {/* Navbar Links */}
          {NavbarMenu.map((navbar) => (
            <Link
              className={"text-[#2ecc71] dark:text-[#2ecc71] font-semibold"}
              href={navbar.link}
              key={navbar.name}
            >
              <div className='h-full pb-1 hover:pb-0 px-2 flex items-center hover:border-b-4  border-[#2ecc71] dark:border-[#2ecc71] transition-all'>
                {navbar.name}
              </div>
            </Link>
          ))}
        ...
        <div className='flex items-center gap-4'>
          <button
            className='text-xl text-[#2ecc71] dark:text-[#2ecc71] hover:scale-110'
            onClick={setThemeFun}
          >
            {theme === "dark" ? (
              <TbBulbFilled />
